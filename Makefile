...
test-ui:
	...
	$(MAKE) test-with-database STABILITY_TEST=1 TIMEOUT_M=25 PROVE_ARGS="$$HARNESS t/ui/*.t" GLOBIGNORE="t/*tidy*:t/*compile*:$(unstables)"
...
test-fullstack-unstable:
	$(MAKE) test-with-database FULLSTACK=1 TIMEOUT_M=15 PROVE_ARGS="$$HARNESS t/05-scheduler-full.t t/33-developer_mode.t" RETRY=20
...
test-unit-and-integration:
	...
	RETRY=${RETRY} STABILITY_TEST=1 HOOK=./tools/delete-coverdb-folder timeout -s SIGINT -k 5 -v ${TIMEOUT_RETRIES} tools/retry prove ${PROVE_LIB_ARGS} ${PROVE_ARGS}
||||||| parent of 7e79d31dc... [WIP] Check for stability of ALL tests
