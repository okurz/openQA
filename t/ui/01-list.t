...
subtest 'filter-finished' => sub {
    ...
    $driver->execute_script('var s = document.getElementById("finished-jobs-result-filter"); Array.from(s.options).forEach(o => o.selected = (o.value.toLowerCase() === "passed")); s.dispatchEvent(new Event("change"));');
    ...
    $driver->execute_script('var s = document.getElementById("finished-jobs-result-filter"); Array.from(s.options).forEach(o => o.selected = false); s.dispatchEvent(new Event("change"));');
