...
sub send_message_to_java_script_clients ($self, $job_id, $type, $what, $data, $status_code_to_quit_on_finished = undef)
{
...
sub send_message_to_java_script_clients_and_finish ($self, $job_id, $type, $what, $data, $status_code = undef) {
...
sub send_message_to_java_script_client_and_finish ($self, $java_script_tx, $type, $what, $data, $status_code = undef) {
    # set status code (errors or normal quit)
    $status_code = ($type eq 'error' ? 1011 : 1000) unless ($status_code);
