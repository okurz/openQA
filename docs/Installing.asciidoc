...
openQA can automatically trigger jobs for investigation of failed, unlabeled
tests to
allow the test reviewers to more easily bisect and find out most likely failure
reasons. As each failed job will trigger multiple, automatic clones this can
have significant impact on the job execution load but can save manual
investigation and retriggering work
